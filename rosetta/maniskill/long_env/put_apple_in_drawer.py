...
class PutAppleInDrawerEnv(BaseEnv):
    ...
    def evaluate(self):
        ...
        if self.cur_stage==2:
            info["success"] = info["stage2_success"]
    ...
    def task_fail(self, info: Dict):
        # if (not info['is_apple_in_drawer']) and info['is_drawer_closed']:
        #     return True
        # apple_pos = info["apple_pos"]
        # if (apple_pos[0] < self.workspace_x[0] or apple_pos[0] > self.workspace_x[1] or
        #     apple_pos[1] < self.workspace_y[0] or apple_pos[1] > self.workspace_y[1] or
        #     apple_pos[2] < 0):
        #     return True
        return False
    ...
    def skill_reward(self, prev_info, cur_info, action, **kwargs):
        ...
        def stage_2_reward():
            target_action = 2  # push
            target_pos_1 = prev_info['drawer_handle_pos']
            target_pos_2 = target_pos_1 + np.array([0, -0.15, 0])

            if current_selected_action == target_action:
                pos_error = (np.linalg.norm(current_selected_pos_1 - target_pos_1) +
                            np.linalg.norm(current_selected_pos_2 - target_pos_2))
                reward = - np.tanh(pos_error)
                reward_components["afford"] = (1 + reward) * 5
