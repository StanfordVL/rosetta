...
def rosetta_lh(
    ...
    **kwargs,
    ...
    grounding_components = ground_preference(
        demo_dir, human_input, env_id, act_space, client, task_description
    )
    ...
    system_msg = PromptMessage(
        role="system", content=get_prompt_content(f"{content_version}/fr_system")
    )
    ...
    user_plan_msg = PromptMessage(
        role="user", content=get_prompt_content(f"{content_version}/fplan_user")
    )
    ...
    user_code_msg = PromptMessage(
        role="user", content=get_prompt_content(f"{content_version}/fcode_user")
    )
    ...
    latest_interim_funcs = update_latest_interim_funcs_actprim(
        asst_code_msg, latest_interim_funcs, target_actions
    )
    ...
    user_geom_msg = PromptMessage(
        role="user",
        content=get_prompt_content(f"{content_version}/fcode_geometrycot_user"),
    )
    user_geom_msg.fill_dynamic_fields({"setup_description": env_info.setup_description})
    ...
    latest_interim_funcs = update_latest_interim_funcs_actprim(
        asst_geom_msg, latest_interim_funcs, target_actions
    )
    latest_funcs = build_actprim_funcs(latest_interim_funcs, target_actions)
    all_funcs.append(latest_funcs.copy())

    # Step 14: add normalization review user message to history
    user_norm_msg = PromptMessage(
        role="user", content=get_prompt_content(f"{content_version}/fcode_normcot_user")
    )
    ...
    latest_interim_funcs = update_latest_interim_funcs_actprim(
        asst_norm_msg, latest_interim_funcs, target_actions
    )
    latest_funcs = build_actprim_funcs(latest_interim_funcs, target_actions)
    all_funcs.append(latest_funcs.copy())

    # Step 17: add code cleanup review user message to history
    user_clean_msg = PromptMessage(
        role="user",
        content=get_prompt_content(f"{content_version}/fcode_codecleanupcot_user"),
    )
    ...
    latest_interim_funcs = update_latest_interim_funcs_actprim(
        asst_clean_msg, latest_interim_funcs, target_actions
    )
    latest_funcs = build_actprim_funcs(latest_interim_funcs, target_actions)
    all_funcs.append(latest_funcs.copy())
